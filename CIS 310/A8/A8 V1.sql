...
SELECT	A.ITEMID, AverageCost, AverageSalePrice
FROM	(SELECT ITEMID, AVG(SALEPRICE) AS AverageSalePrice FROM PET..SaleItem
		GROUP BY ITEMID) A INNER JOIN 
		(SELECT ITEMID, AVG(COST) AS AverageCost FROM  PET..ORDERITEM
		GROUP BY ITEMID) B ON A.ITEMID=B.ITEMID
WHERE	A.AverageSalePrice>1.5*B.AverageCost
...
FROM PET..SUPPLIER S INNER JOIN PET..MERCHANDISEORDER MO ON S.SUPPLIERID=MO.SUPPLIERID 
 ...
 SELECT AVG(SUPPLIERTOTAL.SUPPLIERMERCHTOTAL)
 FROM SUPPLIERTOTAL


 --SHOWS PO WITH SUPID AND TOTAL
SELECT OI.PONUMBER, MO.SUPPLIERID , SUM(QUANTITY*COST) AS MERCHTOTAL
FROM PET..SUPPLIER S INNER JOIN PET..MERCHANDISEORDER MO ON S.SUPPLIERID = MO.SUPPLIERID INNER JOIN
	PET..ORDERITEM OI ON MO.PONUMBER = OI.PONUMBER
GROUP BY OI.PONUMBER, MO.SUPPLIERID
...
--CUSTOMERS WHO BOUGHT MORE THAN $100 ON MERCH IN MAY
CREATE VIEW MAY100 AS
SELECT C.CUSTOMERID, C.LASTNAME, C.FIRSTNAME, S.SALEDATE, SUM(SI.QUANTITY *SI.SALEPRICE) AS TOTAL
...
--CUSTOMERS WHO BOUGHT MORE THAN $50 ON MERCH IN OCT
CREATE VIEW OCT50 AS
SELECT C.CUSTOMERID, C.LASTNAME, C.FIRSTNAME, S.SALEDATE, SUM(SI.QUANTITY *SI.SALEPRICE) AS TOTAL
FROM PET..SALEITEM SI INNER JOIN PET.. SALE S ON SI.SALEID = S.SALEID 
	INNER JOIN PET..CUSTOMER C ON S.CUSTOMERID = C.CUSTOMERID 
GROUP BY C.CUSTOMERID, C.LASTNAME, C.FIRSTNAME, S.SALEDATE
...
--JOINS BOTH TABLE TO FIND CUSTOMER WITH BOTH CONDITIONS	
SELECT M.CUSTOMERID, M.LASTNAME, M.FIRSTNAME, M.TOTAL AS MAYTOTAL, O.TOTAL AS OCTTOTAL
FROM  MAY100 M INNER JOIN OCT50 O ON M.CUSTOMERID= O.CUSTOMERID 
